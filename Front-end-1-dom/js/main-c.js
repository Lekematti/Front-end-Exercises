...
for (let i = 0; i < picArray.length; i++) {
  ...
  let newHeaderH2 = document.createElement('h2');
  newHeaderH2.innerText = picArray[i].title;

  newHeader.appendChild(newHeaderH2);

  let newFig = document.createElement('figure');
  let newImg = document.createElement('img');
  newImg.src = picArray[i].filename;

  let newFigCapt = document.createElement('figurecaption');
  newFigCapt.innerText = picArray[i].caption;

  newFig.appendChild(newImg);
  newFig.appendChild(newFigCapt);

  let newText = document.createTextNode(picArray[i].description);
